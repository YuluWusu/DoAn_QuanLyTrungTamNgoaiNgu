...
ON PRIMARY 
(
    NAME = 'QL_TRUNGTAM_TIENGANH_Data',
    FILENAME = 'C:\CSDL\QL_TRUNGTAM_TIENGANH_Data.mdf', 
    SIZE = 50MB,      
    MAXSIZE = 500MB,   
    FILEGROWTH = 10MB    
)
LOG ON 
(
    NAME = 'QL_TRUNGTAM_TIENGANH_Log', 
    FILENAME = 'C:\CSDL\QL_TRUNGTAM_TIENGANH_Log.ldf', 
    SIZE = 20MB,          
    MAXSIZE = 200MB,     
    FILEGROWTH = 5MB     
);
...
USE master;
GO
ALTER DATABASE QL_TRUNGTAM_TIENGANH SET RECOVERY FULL;
GO
...
-- SAO LƯU
-- 1. Chạy Full Backup
BACKUP DATABASE QL_TRUNGTAM_TIENGANH
TO DISK = 'D:\CSDL_BACKUP\QLTTTA_FULL.bak'
WITH INIT, NAME = N'QL_TRUNGTAM_TIENGANH Full Backup',
DESCRIPTION = N'Sao lưu toàn bộ dữ liệu trung tâm tiếng Anh';
GO
-- 2. Chạy Differential Backup (Lưu những gì thay đổi từ bản Full)
BACKUP DATABASE QL_TRUNGTAM_TIENGANH
TO DISK = 'D:\CSDL_BACKUP\QLTTTA_DIFF.bak'
WITH DIFFERENTIAL, NAME = N'QL_TRUNGTAM_TIENGANH Differential Backup';
PRINT N'Đã hoàn thành sao lưu Differential Backup';
GO
-- 3. Chạy Log Backup (Lưu nhật ký giao dịch)
BACKUP LOG QL_TRUNGTAM_TIENGANH
TO DISK = 'D:\CSDL_BACKUP\QLTTTA_LOG.trn'
WITH NAME = N'QL_TRUNGTAM_TIENGANH Log Backup';
PRINT N'Đã hoàn thành sao lưu Nhật ký (Log Backup)';
GO
USE master;
GO

-- PHỤC HỒI
-- Ngắt các kết nối đang dùng CSDL để có thể Restore
ALTER DATABASE QL_TRUNGTAM_TIENGANH SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
GO

-- BƯỚC 4.1: Phục hồi bản Full Backup gần nhất (Dùng NORECOVERY để hệ thống chờ bản kế tiếp)
RESTORE DATABASE QL_TRUNGTAM_TIENGANH
FROM DISK = 'D:\CSDL_BACKUP\QLTTTA_FULL.bak'
WITH NORECOVERY, REPLACE;
GO

-- BƯỚC 4.2: Phục hồi bản Differential Backup
RESTORE DATABASE QL_TRUNGTAM_TIENGANH
FROM DISK = 'D:\CSDL_BACKUP\QLTTTA_DIFF.bak'
WITH NORECOVERY;
GO

-- BƯỚC 4.3: Phục hồi bản Transaction Log cuối cùng (Dùng RECOVERY để đưa DB về trạng thái hoạt động)
RESTORE LOG QL_TRUNGTAM_TIENGANH
FROM DISK = 'D:\CSDL_BACKUP\QLTTTA_LOG.trn'
WITH RECOVERY;
GO

-- Đưa CSDL về lại chế độ đa người dùng
ALTER DATABASE QL_TRUNGTAM_TIENGANH SET MULTI_USER;
GO
